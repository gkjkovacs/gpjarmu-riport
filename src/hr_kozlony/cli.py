"""Typer CLI for the HR Középvállalati Magyar Közlöny Havi Riport.

Commands:
    run              Run the monthly Magyar Közlöny monitor pipeline.
    seed             Initial 30-day seed run.
    init-db [--force]  Initialize or reset the state database.
    show-config      Print the current configuration (redacted).
    show-state       Inspect the state database.

This module is the single source of truth for the CLI. It is importable
as `hr_kozlony.cli:app` (Typer application) so that `pip install` produces
a working `hr-kozlony` console script, and the legacy `run.py` wrapper
just re-exports `app` for users who run `python run.py …` directly.
"""
...
app = typer.Typer(
    name="hr-kozlony",
    help="Havi HR középvállalati Magyar Közlöny monitor.",
